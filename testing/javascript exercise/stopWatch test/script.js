...
const miniNumber = document.getElementsByClassName("smallOnes")[0];
...
startButtonTag.addEventListener("click",() => {
     intervalId = setInterval(startTime,startMini,1000)
     intervalMini = setInterval(startMini,100)
})
...
continueButtonTag.addEventListener("click", () => {
 ...
 let numbers = 0;
let startMini = () => {
    miniNumber.textContent = numbers;
    console.log("counting:",numbers)
    
}

startButtonTag.addEventListener("click", ()=> {
    setInterval(startMini,10)
})
